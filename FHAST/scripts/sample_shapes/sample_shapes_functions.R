################################################################################
# Take a shape file and a grid and samples a specified column from
...
sample_shape_with_grid <- function(grid, shape_file, column_name, output_name) {
  ...
  grid <- grid %>%
    select(dist, area, l_or_r, geometry, lat_dist) %>%
    rowid_to_column("ID")
  ...
  if(NROW(grid_samples_check)==0){
    ...
    message(paste0("The ", output_name, " shape does not overlap the grid.\nPlease confirm that this is expected.\n"))
  }
...
sample_all_shapes <- function(grid = NULL,
  ...
  output <- future_pmap(
    ...
    reduce(~ left_join(.x, .y, by = c("ID", "dist", "area", "l_or_r", "geometry", "lat_dist"))) %>%
