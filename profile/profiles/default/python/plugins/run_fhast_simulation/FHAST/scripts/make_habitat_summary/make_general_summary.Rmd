...
summary_stats = readRDS(file = here(output_temp_folder,  "summary_stats_table.rds")) 

summary_stats_cutoff = readRDS(file = here(output_temp_folder,  "summary_stats_cutoff_table.rds")) 

fish_energy_area_stats = readRDS(file = here(output_temp_folder,  "fish_energy_area_stats_table.rds")) 

habitat_parm = readRDS(file = here(output_temp_folder,  "habitat_parm.rds")) 

fish_summary_stats = readRDS(file = here(output_temp_folder,  "summary_fish_stats_table.rds"))

cover_scatter_plot = here(output_temp_folder,  "cover_scatter_plot.png")

cover_cutoff_map = here(output_temp_folder,  "cover_cutoff_map.png")

cover_facet_hist = here(output_temp_folder,  "cover_facet_hist.png")

cutoff_map = here(output_temp_folder,  "cutoff_map.png")

heat_map_plot = here(output_temp_folder,  "depth_velocity_heatmap.png")

fish_met_plot = here(output_temp_folder,  "metabolic_map.png")

fish_net_energy_plot = here(output_temp_folder,  "net_energy_map.png")

```

```{r pre_rrocess, include=FALSE}
predator_list =  readRDS(here(output_temp_folder, "pred_list.rds"))
fish_list = readRDS(here(output_temp_folder, "fish_list.rds"))
n = 0
t = 0 
```
...
Notes
```{r, echo=FALSE, }
cat(readLines(here(output_temp_folder, "notes.txt")), sep = '\n')
```
***
## Tabular Outputs 
...
The main input file:
```{r, echo=FALSE}
cat(readLines(file.path(here(output_folder, "input_file.txt"))), sep = '\n')
```
---

The daily input file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "daily_input.txt"))), sep = '\n')
```
---

The interaction input file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "interactions_input.txt"))), sep = '\n')
```
---

The fish population file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "population_input.txt"))), sep = '\n')
```
---

The fish parameter file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "fish_input.txt"))), sep = '\n')
```
---

The habitat parameter file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "habitat_input.txt"))), sep = '\n')
```
---

The predator input file:
```{r, echo=FALSE, }
cat(readLines(file.path(here(output_temp_folder,  "predator_input.txt"))), sep = '\n')
```
---
</font>
